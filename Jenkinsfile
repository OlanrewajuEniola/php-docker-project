pipeline {
    agent { label 'slave' }
    options {
        // IMPORTANT: This cleans the workspace before each build.
        // It ensures no leftover 'hosts' file (or other artifacts) from previous runs cause conflicts.
        cleanWs()
    }

    stages {
        // REMOVE THE MANUAL "Clone or Pull GitHub Repo" STAGE COMPLETELY.
        ...
        stage('Prepare Ansible Hosts') {
            steps {
                // Write the 'hosts' file directly into the Jenkins workspace (which is the current directory here for this stage).
                sh '''
                    echo "[slaves]" > hosts
        ...
        stage('Run Ansible to Install Docker') {
            steps {
                // 'hosts' and 'install-docker.yaml' are in the workspace root, so no 'cd' or path prefix needed.
                sh '''
                    sudo apt-get update
        ...
        stage('Build and Deploy Docker Container') {
            steps {
                // 'Dockerfile' is in the workspace root.
